...
export default function NutritionProgress({ user, todayTotals }) {
  const macros = [
    { name: "Calories", current: Math.round(todayTotals.calories), target: user?.target_calories || 2000, icon: Zap, color: "emerald", unit: "" },
    { name: "Protein", current: Math.round(todayTotals.protein), target: user?.target_protein || 150, icon: Beef, color: "blue", unit: "g" },
    { name: "Carbs", current: Math.round(todayTotals.carbs), target: user?.target_carbs || 250, icon: Wheat, color: "amber", unit: "g" },
    { name: "Fat", current: Math.round(todayTotals.fat), target: user?.target_fat || 70, icon: Target, color: "purple", unit: "g" }
  ];
